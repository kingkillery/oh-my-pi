...
mod tests {
	...
	fn bun_run_check_colon_compacts_workspace_success_noise() {
		...
		assert!(out.text.contains("@pk-nerdsaver-ai/pi-utils"));
		assert!(out.text.contains("@pk-nerdsaver-ai/pi-coding-agent"));
	...
	fn bun_check_failure_keeps_diagnostic_and_emits_failed_status() {
		...
		let input = concat!(
			...
			"@pk-nerdsaver-ai/pi-utils check: $ tsgo -p tsconfig.json --noEmit\n",
			"@pk-nerdsaver-ai/pi-utils check: Exited with code 0\n",
			"@pk-nerdsaver-ai/pi-coding-agent check: $ tsgo -p tsconfig.json --noEmit\n",
			"src/tools/bash.ts(42,7): error TS2322: Type 'string' is not assignable to type \
			 'number'.\n",
			"@pk-nerdsaver-ai/pi-coding-agent check: Exited with code 1\n",
		);
	...
	fn bun_check_success_emits_passed_status_and_no_noise() {
		...
		let input = concat!(
			...
			"@pk-nerdsaver-ai/pi-utils check: Checked 40 files in 11ms. No fixes applied.\n",
			"@pk-nerdsaver-ai/pi-utils check: $ tsgo -p tsconfig.json --noEmit\n",
			"@pk-nerdsaver-ai/pi-utils check: Exited with code 0\n",
			"@pk-nerdsaver-ai/pi-coding-agent check: Checked 1178 files in 287ms. No fixes applied.\n",
			"@pk-nerdsaver-ai/pi-coding-agent check: $ tsgo -p tsconfig.json --noEmit\n",
			"@pk-nerdsaver-ai/pi-coding-agent check: Exited with code 0\n",
		);
