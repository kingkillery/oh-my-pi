...
`src/extensibility/hooks/index.ts` and the package subpath `@pk-nerdsaver-ai/pi-coding-agent/extensibility/hooks` export:
...
The package root (`@pk-nerdsaver-ai/pi-coding-agent`) does not re-export `HookAPI`; import legacy hook types from the hooks subpath.
