...
This document defines the JS/TS contract between `@pk-nerdsaver-ai/pi-natives` callers and the loaded N-API addon.
...
Consumers in `packages/coding-agent` and `packages/tui` import directly from `@pk-nerdsaver-ai/pi-natives`.
