 ...
 * `@mariozechner` (original), `@earendil-works` (fork), and `@pk-nerdsaver-ai`
...
const canonicalCodingAgent = Bun.resolveSync("@pk-nerdsaver-ai/pi-coding-agent", import.meta.dir);
const canonicalCodingAgentExtensions = Bun.resolveSync(
	"@pk-nerdsaver-ai/pi-coding-agent/extensibility/extensions",
	import.meta.dir,
);
const canonicalUtils = Bun.resolveSync("@pk-nerdsaver-ai/pi-utils", import.meta.dir);
const canonicalTui = Bun.resolveSync("@pk-nerdsaver-ai/pi-tui", import.meta.dir);
// Subpath: upstream `pi-ai/oauth` re-exported `utils/oauth/index`; our pi-ai now
// exposes the same surface at the real `@pk-nerdsaver-ai/pi-ai/oauth` export, so the
// legacy `@mariozechner/pi-ai/oauth` specifier canonicalizes straight to it.
const canonicalAiOauth = Bun.resolveSync("@pk-nerdsaver-ai/pi-ai/oauth", import.meta.dir);
...
const CASES: readonly AliasCase[] = [
	...
	// @pk-nerdsaver-ai self-import — canonical scope must still flow through the shim
	// so a duplicate copy is never dragged in from a plugin's own node_modules.
	{ id: "ohmypi-utils", aliasSpecifier: "@pk-nerdsaver-ai/pi-utils", canonicalPath: canonicalUtils, symbol: "logger" },
	{
		id: "ohmypi-coding-agent",
		aliasSpecifier: "@pk-nerdsaver-ai/pi-coding-agent",
	...
	// Subpath: legacy `pi-ai/oauth` resolves to the real `@pk-nerdsaver-ai/pi-ai/oauth`.
	{
