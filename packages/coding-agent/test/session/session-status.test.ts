...
import { isBackgroundInstanceSession, type SessionStatus } from "@oh-my-pi/pi-coding-agent/session/session-listing";
...
function headerWithBackground(id: string, status: "active" | "archived"): string {
	return line({
		type: "session",
		version: 3,
		id,
...
const background = (status: "active" | "archived") =>
	line({
...
function seedRaw(files: Record<string, string>): MemorySessionStorage {
	const storage = new MemorySessionStorage();
	for (const id in files) {
		storage.writeTextSync(`${SESSION_DIR}/${id}.jsonl`, files[id]);
	}
	return storage;
}
...
describe("SessionManager.list background instances", () => {
	it("surfaces active background instance metadata", async () => {
		const storage = seed({ active: background("active") + user("continue work") });

		const sessions = await SessionManager.list("/proj", SESSION_DIR, storage);
		const session = sessions.find(item => item.id === "active");

		expect(session?.backgroundInstance).toEqual({
			name: "api-worker",
			status: "active",
			model: "anthropic/claude-sonnet-4-6",
			role: "default",
	...
	it("treats a later archived background entry as inactive", async () => {
		...
		const sessions = await SessionManager.list("/proj", SESSION_DIR, storage);
		const session = sessions.find(item => item.id === "archived");

		expect(session?.backgroundInstance).toBeUndefined();
		expect(session ? isBackgroundInstanceSession(session) : false).toBe(false);
	});

	it("keeps active background metadata after the marker scrolls out of listing slices", async () => {
		const storage = seedRaw({
			long:
				headerWithBackground("long", "active") +
				user("start") +
				"x".repeat(5_000) +
				background("active") +
				"y".repeat(40_000),
		...
		const sessions = await SessionManager.list("/proj", SESSION_DIR, storage);
		const session = sessions.find(item => item.id === "long");
	...
	it("does not resurrect active entries when the header cache is archived", async () => {
		const storage = seedRaw({
			archivedHeader: headerWithBackground("archivedHeader", "archived") + background("active"),
		});

		const sessions = await SessionManager.list("/proj", SESSION_DIR, storage);
		const session = sessions.find(item => item.id === "archivedHeader");
