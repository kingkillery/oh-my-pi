...
import { isBackgroundInstanceSession, type SessionInfo } from "../../session/session-listing";
...
export class SelectorController {
	...
	async showBackgroundInstanceSelector(): Promise<void> {
		const sessions = await SessionManager.list(
			this.ctx.sessionManager.getCwd(),
			this.ctx.sessionManager.getSessionDir(),
		);
		const backgroundSessions = sessions.filter(isBackgroundInstanceSession);
		let allSessions: SessionInfo[] | undefined;
		let startInAllScope = false;
		if (backgroundSessions.length === 0) {
			allSessions = await SessionManager.listAll();
			startInAllScope = allSessions.some(isBackgroundInstanceSession);
		}
		const historyStorage = this.ctx.historyStorage;
		const historyMatcher = historyStorage ? (query: string) => historyStorage.matchingSessionIds(query) : undefined;
		this.showSelector(done => {
