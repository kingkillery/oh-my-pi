...
import { formatBytes } from "@pk-nerdsaver-ai/pi-utils";
...
function sessionSearchText(session: SessionInfo): string {
	const parts = [
		session.id,
		session.backgroundInstance?.name ?? "",
		session.backgroundInstance?.model ?? "",
		session.title ?? "",
		session.cwd ?? "",
...
class SessionList implements Component {
	...
	readonly #mode: "sessions" | "backgroundInstances";
	...
	constructor(
		...
		mode: "sessions" | "backgroundInstances" = "sessions",
	) {
	...
	render(width: number): readonly string[] {
		...
		for (let i = startIndex; i < endIndex; i++) {
			...
			if (this.#mode === "backgroundInstances" && session.backgroundInstance?.model) {
				metadata += ` ${dot} ${dim(`model ${session.backgroundInstance.model}`)}`;
			}
		...
		lines.push(
			theme.fg(
				"muted",
				`  [${deleteHint}Enter select · Tab ${this.#showCwd ? "current folder" : "all projects"} · Esc cancel]`,
			),
		);
...
export interface SessionSelectorOptions {
	...
	mode?: "sessions" | "backgroundInstances";
}
...
export class SessionSelectorComponent extends Container {
	...
	constructor(
		...
		const initialSessions = startAll ? this.#globalSessions! : this.#folderSessions;
		// Add header
	...
	#filterSessionsForMode(sessions: SessionInfo[]): SessionInfo[] {
		return this.#mode === "backgroundInstances" ? sessions.filter(isBackgroundInstanceSession) : sessions;
	}

	#headerLabel(): string {
		const scopeLabel = this.#scope === "all" ? "all projects" : "current folder";
		const label = this.#mode === "backgroundInstances" ? "Background Agents" : "Resume Session";
		return `${theme.bold(label)} ${theme.fg("muted", `(${scopeLabel})`)}`;
	}
	...
	async #toggleScope(): Promise<void> {
		...
		if (this.#scope === "folder") {
			...
			if (!global) {
				...
				try {
					global = this.#filterSessionsForMode(await this.#loadAllSessions());
				} catch (err) {
