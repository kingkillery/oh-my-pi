...
const BUILTIN_SLASH_COMMAND_REGISTRY: ReadonlyArray<SlashCommandSpec> = [
	...
	{
		name: "background",
		description: "Save this session as a named background agent and open the background switcher",
		inlineHint: "[name]",
		allowArgs: true,
		handleTui: async (command, runtime) => {
			...
			runtime.ctx.editor.setText("");
			await runtime.ctx.showBackgroundInstanceSelector();
		},
	},
	{
		name: "backgrounds",
		aliases: ["agents"],
		description: "Open background agent switcher",
		handleTui: async (_command, runtime) => {
			runtime.ctx.editor.setText("");
			await runtime.ctx.showBackgroundInstanceSelector();
		},
