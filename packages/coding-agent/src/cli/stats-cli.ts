...
import { truncateToWidth } from "@pk-nerdsaver-ai/pi-tui/utils";
import { APP_NAME, formatDuration, formatNumber, formatPercent } from "@pk-nerdsaver-ai/pi-utils";
...
async function printStatsSummary(): Promise<void> {
	const { getDashboardStats } = await import("@pk-nerdsaver-ai/omp-stats");
	const stats = await getDashboardStats();
