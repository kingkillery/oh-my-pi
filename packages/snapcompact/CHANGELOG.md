...
- Added package-level TypeScript exports and publication config so consumers can import `@pk-nerdsaver-ai/snapcompact` with typed access to snapcompact APIs
- Published `@pk-nerdsaver-ai/snapcompact` as the reusable snapcompact compaction package, including bitmap-frame rendering helpers, archive helpers, and the local `snapcompactCompact()` strategy.
