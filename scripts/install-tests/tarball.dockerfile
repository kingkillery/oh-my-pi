...
for pkg in "${PACKAGES[@]}"; do
    ...
    echo "Dependencies:"
    jq '.dependencies | to_entries[] | select(.value | startswith("@pk-nerdsaver-ai") or startswith("workspace"))' package.json 2>/dev/null || true
...
RUN verdaccio --config /root/.config/verdaccio/config.yaml &>/dev/null & \
    sleep 3 && \
    bun add @pk-nerdsaver-ai/pi-coding-agent --registry http://localhost:4873 && \
    pkill -f verdaccio
