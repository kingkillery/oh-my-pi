...
const FALLBACK_REPO = "kingkillery/oh-my-pi";
	...
	for (const pkg of packages) {
		...
		if (pkg.kind === "native") {
			for (const target of LEAF_TARGETS) {
				const leaf = `@pk-nerdsaver-ai/pi-natives-${target.tag}`;
				if (!seen.has(leaf)) {
					seen.add(leaf);
					names.push(leaf);
				}
...
function nativeLeafName(tag: string): string {
	return `@pk-nerdsaver-ai/pi-natives-${tag}`;
}
...
function placeholderManifest(name: string, target: NativeLeafTarget, repo: string): PlaceholderManifest {
	return {
		...
		description: `Placeholder for the ${target.tag} native addon of @pk-nerdsaver-ai/pi-natives. The real binary is published during release.`,
		license: "MIT",
		os: [target.os],
		cpu: [target.cpu],
		repository: {
...
function placeholderReadme(name: string, target: NativeLeafTarget): string {
	return [
		`# ${name}`,
		"",
		`Placeholder package reserving the npm name for the \`${target.tag}\` native addon of \`@pk-nerdsaver-ai/pi-natives\`.`,
		"",
